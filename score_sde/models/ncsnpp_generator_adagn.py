...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=0, groups=8, para=15):
        ...
        self.SFT1 = SFT_Layer(nf=dim_out, para=para)
        self.block2 = Block(dim_out, dim_out, groups=groups)
        self.SFT2 = SFT_Layer(nf=dim_out, para=para)
        self.res_conv = nn.Conv2d(dim, dim_out, 1) if dim != dim_out else nn.Identity()

    def forward(self, x, kernel, time_emb=None, cond=None):
        h = self.block1(x)
        h = self.SFT1(h, kernel)
...
class Unet(nn.Module):
    def __init__(self, opt, dim, out_dim=None, dim_mults=(1, 2, 4, 8), cond_dim=32, kernel_size=21, input_para=128):
        super().__init__()
...
#             self.apply(initialize_weights)
    ...
    def forward(self, x, time, img_lr, img_lr_up, lr_t):
#         print(lr_t.size())
        #######首先估计模糊核######
        B,_,_,_ =img_lr.size()
#         kernel = self.Estimator(img_lr)####kernel就等价于原来的psf
        kernel1 = lr_t.view(B,self.ksize*self.ksize,1,1)
        kernel_pca_code = (kernel1.unsqueeze(2) * self.pca_matrix).sum(1, keepdim=False)
...
#         rrdb_out, cond = self.rrdb(img_lr, True)
        ...
        for i, (resnet, resnet2, downsample) in enumerate(self.downs):
            x = resnet(x, kernel_pca_code, t)
            x = resnet2(x, kernel_pca_code, t)
            if i == 0:
                x = x + cond
#                 if opt['res'] and opt['up_input']:
#                     x = x + self.up_proj(img_lr_up)
        ...
        x = self.mid_block1(x, kernel_pca_code, t)
#         if opt['use_attn']:
#             x = self.mid_attn(x)
        x = self.mid_block2(x, kernel_pca_code, t)
