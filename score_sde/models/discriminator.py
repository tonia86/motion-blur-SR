...
class Discriminator_small(nn.Module):
  ...
  def __init__(self, nc = 3, ngf = 64, t_emb_dim = 128, act=nn.LeakyReLU(0.2)):
    ...
    self.t_embed = TimestepEmbedding(#位置编码，全连接层-激活层-全连接层
    ...
    self.final_conv = conv2d(ngf*8 + 1, ngf*8, 3,padding=1, init_scale=0.) #Global Sum Pooling
  ...
  def forward(self, x, t, x_t):
    t_embed = self.act(self.t_embed(t))  #对时间t进行编码
    
  
    input_x = torch.cat((x, x_t), dim = 1)#按通道数进行拼接
    
    h0 = self.start_conv(input_x) #1x1的卷积
    h1 = self.conv1(h0,t_embed) #ResBlock，128
    
    h2 = self.conv2(h1,t_embed) #ResBlock down，256
   
    h3 = self.conv3(h2,t_embed) #ResBlock down，512
   
    
    out = self.conv4(h3,t_embed) #ResBlock down，512
    
    batch, channel, height, width = out.shape
    group = min(batch, self.stddev_group) #取最小值
    stddev = out.view(
    ...
    out = self.end_linear(out) #全连接层 512->1
